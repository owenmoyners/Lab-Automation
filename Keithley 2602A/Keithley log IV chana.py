...
points = 1000 #how many points you want to take

#Path for file to be saved 
file_name = 'A4015 TP EAM PIN diode log IV anulus con light off'
...
end_current = np.log10(end_current) #getting logspace values 
r_path=  repr(path)[1:-1] #changing path to raw string to avoid back slash error

I = np.logspace(start_current, end_current, num=points, base=10) #increasing I in log scale
V = []
...
#plot IV 
plt.plot(V, abs(I), linewidth = 0.4)
plt.yscale("log")
